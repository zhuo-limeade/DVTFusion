...
model = dict(
    ...
    img_view_transformer=dict(
        type='LSSViewTransformer_Pillar_Sparse',
        grid_config=grid_config,
        input_size=data_config['input_size1'],
    ...
    pre_process=dict(
        type='CustomResNet',
        numC_input=feat_bev_img_dim,
        num_layer=[2,],
        num_channels=[feat_bev_img_dim,],
        stride=[1,],
        backbone_output_ids=[0,]),
    bev_after_process=dict(
        type='CustomResNet',
        numC_input=feat_bev_img_dim*2,
        num_layer=[2,],
        num_channels=[feat_bev_img_dim,],
        stride=[1,],
        backbone_output_ids=[0,]),
    ...
    pts_voxel_layer=dict(
        max_num_points=32, voxel_size=voxel_size,
...
optimizer = dict(type='AdamW', lr=3e-4, weight_decay=0.01)  # for 64 total batch size
two_stage = True
