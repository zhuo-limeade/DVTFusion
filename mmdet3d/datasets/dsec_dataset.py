...
class DSECInferenceDataset(DatasetTemplate):
    """
    The Dataset class for Inference on DSEC
    """
...
class DSECTrainingDataset(DatasetTemplate):
    """
    The Dataset class for Training on DSEC (from File System)
    """
    ...
    def init_infos(self):
        ...
        self.logger.info('Loading DSEC dataset')
        waymo_infos = []
        num_skipped_infos = 0
        ...
        # for k in range(10):
            ...
            if self.mode == 'train':
                ...
                # info_path = os.path.join(self.data_path, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_interpolate_lidar_check.pkl')      
            else:
                info_path = os.path.join(self.data_path, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_interpolate_fov_bbox_lidar_check.pkl')    
                # info_path = os.path.join(self.data_path, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_fov_bbox.pkl')    
                # info_path = os.path.join(self.data_path, sequence_name, ('%s.pkl' % sequence_name)).replace('.pkl', '_interpolate_lidar_check.pkl')      
            ...
            if self.mode == 'train':
                ...
                for i in range(len(infos)):
                    ...
                    for j in range(9):
                        event_paths.append(os.path.join(event_init_path, event_file_name + '_' + str(j+1) + '.npz'))
                    # pdb.set_trace()
    ...
    def get_infos_and_points(self, idx_list):
        ...
        for i in idx_list:
            ...
            if 's3' in self.root_path:
                ...
                disparity_path = os.path.join(self.data_path, self.infos[i]['disparity_path'])
            ...
            for key in img_infos.keys():
                ...
                for j in range(1):
                    ...
                    if 's3' in self.root_path:
                        ...
                        # pdb.set_trace()
                        # if not os.path.isfile(img_path): pdb.set_trace()
                        image = cv2.imread(img_path)
                    # normalize images
                    if image is None: pdb.set_trace()
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        # eval_det_annos_first = []
        # for info in self.infos:
        #     annos = copy.deepcopy(info['seq_annos'])
        #     for anno in annos:
        #         eval_det_annos_first.append(annos[0])
        
        # for anno, gt_anno in zip(eval_det_annos, eval_gt_annos):
        #     anno['boxes_lidar'] = gt_anno['gt_boxes_lidar'][:, :7]
        #     anno['name'] = gt_anno['name']
        #     anno['score'] = np.ones(gt_anno['gt_boxes_lidar'].shape[0])
