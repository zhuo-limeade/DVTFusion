...
class LSSViewTransformer_Pillar(BaseModule):
    ...
    def __init__(
        ...
        super(LSSViewTransformer_Pillar, self).__init__()
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        # Lift-Splat
        coor = self.get_lidar_coor(*input[1:7])# [B,N,118,24,68,3]
    ...
    def forward(self, input, depth_from_lidar=None):
        ...
        # depth_topk, depth_indice=torch.topk(depth, k=self.K, dim=2)# 将连续深度离散化, [BN,118,24,68]->[BN,k,24,68]
        
        # return self.view_transform(input, x1, depth_topk, depth_indice)
        return self.view_transform(input, depth, x)
...
@NECKS.register_module()
class LSSViewTransformer_Pillar_1(BaseModule):
    r"""Lift-Splat-Shoot view transformer with BEVPoolv2 implementation.
    ...
    def __init__(
        ...
        super(LSSViewTransformer_Pillar_1, self).__init__()
        self.with_cp = with_cp
        self.grid_config = grid_config
        self.downsample = downsample
        self.create_grid_infos(**grid_config)
    ...
    def voxel_pooling_v2(self, coor, depth, depth_indice, feat):
        ranks_bev, ranks_depth, ranks_feat, \
            interval_starts, interval_lengths = \
            self.voxel_pooling_prepare_v2(coor)
        if ranks_feat is None:
    ...
    def view_transform_core(self, input, depth, depth_indice, tran_feat):
        B, N, C, H, W = input[0].shape
        # Lift-Splat
        coor = self.get_lidar_coor(*input[1:7])# [B,N,118,24,68,3]
        bev_feat = self.voxel_pooling_v2(
            coor, depth.view(B, N, self.K, self.height, W),depth_indice.view(B, N, self.K, self.height, W),
            tran_feat.view(B, N, self.out_channels,  self.height, W))
        return bev_feat, depth                                  

    def view_transform(self, input, depth, depth_indice, tran_feat):
        return self.view_transform_core(input, depth, depth_indice, tran_feat)
    ...
    def forward(self, input, depth_from_lidar=None):
        ...
        depth = depth_digit.softmax(dim=1)
        depth = depth.mean(dim=2, keepdim=True)#对高度轴进行降维[BN,118,24,68]->[BN,118,1,68]
        x = x.mean(dim=2, keepdim=True)#对高度轴进行降维[BN,118,24,68]->[BN,118,1,68]
        # depth_topk, depth_indice=torch.topk(depth, k=self.K, dim=1)# 将连续深度离散化, [BN,118,1,68]->[BN,K,1,68], K=8
        
        # return self.view_transform(input, x1, depth_topk, depth_indice)
        return self.view_transform(input, depth, depth, x)
