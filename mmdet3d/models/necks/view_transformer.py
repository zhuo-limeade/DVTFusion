...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        """
        B, N, _, _ = sensor2ego.shape# 该处生成的是机械的网格，无需进行更改
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ranks_bev, ranks_depth, ranks_feat, \
            interval_starts, interval_lengths = \
            self.voxel_pooling_prepare_v2(coor)# ranks_feat:[N], ranks_depth:[N]# TODO：改为
        if ranks_feat is None:
            ...
            return dummy
        feat = feat.permute(0, 1, 3, 4, 2)# [2, 5, 32, 24, 68]->[2, 5, 24, 68, 32]
        bev_feat_shape = (depth.shape[0], int(self.grid_size[2]),
                          int(self.grid_size[1]), int(self.grid_size[0]),
                          feat.shape[-1])  # (B, Z, Y, X, C)=[2,1,128,128,32]
    ...
    def voxel_pooling_prepare_v2(self, coor):
        ...
        """
        B, N, D, H, W, _ = coor.shape# coor:[B,N,118,24,68,3]
        num_points = B * N * D * H * W
        # record the index of selected points for acceleration purpose
        ranks_depth = torch.range(
            0, num_points - 1, dtype=torch.int, device=coor.device)# [num_points]
        ranks_feat = torch.range(
            0, num_points // D - 1, dtype=torch.int, device=coor.device)# [num_points/D]
        ranks_feat = ranks_feat.reshape(B, N, 1, H, W)
        ranks_feat = ranks_feat.expand(B, N, D, H, W).flatten()
        # convert coordinate into the voxel space
        coor = ((coor - self.grid_lower_bound.to(coor)) /
                self.grid_interval.to(coor))# coor:[B,N,118,24,68,3]
        ...
        interval_lengths[:-1] = interval_starts[1:] - interval_starts[:-1]# 确定进行cumsum的起点和终点
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        if self.accelerate:
            ...
            coor = self.get_lidar_coor(*input[1:7])# [2, 5, 118, 24, 68, 3]
            bev_feat = self.voxel_pooling_v2(
                coor, depth.view(B, N, self.D, H, W),# [2, 5, 118, 24, 68]
                tran_feat.view(B, N, self.out_channels, H, W))# [2, 5, 32, 24, 68]
        return bev_feat, depth

    def view_transform(self, input, depth, tran_feat):
        for shape_id in range(3):
            assert depth.shape[shape_id+1] == self.frustum.shape[shape_id]# self.frustum:[118, 24, 68, 3]
    ...
    def forward(self, input, depth_from_lidar=None):
        ...
        if self.with_depth_from_lidar:
            ...
            if isinstance(depth_from_lidar, list):
                ...
                depth_from_lidar = depth_from_lidar[0]# [2, 5, 192, 544]
            h_img, w_img = depth_from_lidar.shape[2:]
            depth_from_lidar = depth_from_lidar.view(B * N, 1, h_img, w_img)
            depth_from_lidar = self.lidar_input_net(depth_from_lidar)# [10, 64, 24, 68]
            x = torch.cat([x, depth_from_lidar], dim=1)# x:[10, 128, 24, 68]->[10, 192, 24, 68]
        if self.with_cp:# false
            x =checkpoint(self.depth_net, x)
        else:
            x = self.depth_net(x)# [10, 192, 24, 68]->[10, 150, 24, 68]

        depth_digit = x[:, :self.D, ...]# [10, 118, 24, 68]
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]# [10, 32, 24, 68]
