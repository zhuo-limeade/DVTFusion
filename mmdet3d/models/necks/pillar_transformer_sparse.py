...
class LSSViewTransformer_Pillar_Sparse(BaseModule):
    ...
    def __init__(
        ...
        super(LSSViewTransformer_Pillar_Sparse, self).__init__()
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        self.frustrum_temp = self.frustum.to(sensor2ego).expand(B, N, self.K, 1, 68, 3).clone()# [K,1,68,3]->[B,N,K,1,68,3]
        depth_indice = depth_indice.view(B, N, self.K, 1, 68).unsqueeze(-1)# [B,N,K,1,68]->[B,N,K,1,68,1]
    ...
    def view_transform_core(self, input, depth, tran_feat, depth_indice):
        ...
        bev_feat = self.voxel_pooling_v2(
            coor, depth.view(B, N, self.D, 1, W),
            tran_feat.view(B, N, self.out_channels, 1, W))
...
@NECKS.register_module()
class LSSViewTransformer_Pillar_Sparse_with_ATT(LSSViewTransformer_Pillar_Sparse):
    def __init__(self, **kargs):
        super(LSSViewTransformer_Pillar_Sparse_with_ATT, self).__init__(**kargs)
